...
def logger(log_path):
    def decorator(search_countries):
        def new_search_countries(file_path, search):
            date_today = datetime.datetime.now().strftime('%d.%m.%Y %H:%M')
            start = time.time()
            result = search_countries(file_path, search)
            ...
            with open(log_path, 'a', encoding='utf-8') as ouf:
                ...
                ouf.write(f'Аргументы функции - {file_path}, {search}' + '\n')
...
search_countries('countries.json', 'United')
